...
class MetricsCallback(Callback):
    ...
    def _transfer_to_cpu(self, item):
        ...
        if isinstance(item, collections.abc.Mapping):
            return {k: self.__transfer_to_cpu(v) for k, v in item.items()}
        if isinstance(item, collections.abc.Sequence):
            return type(item)(self.__transfer_to_cpu(v) for v in item)
        else:
    ...
    def after_batch(self):
        ...
        y_true = self._transfer_to_cpu(self.learner.batch_context["y"])
        y_pred = self._transfer_to_cpu(self.learner.batch_op["predictions"])
        batch_loss = self._transfer_to_cpu(self.learner.batch_op["loss"])
